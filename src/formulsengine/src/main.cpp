...
void init(int thepatch, int deviceSelect, int channelsOut){  // JD: "int deviveSelect" takes the argument that sets the audio device
   ...
   if (thepatch == 0) {
     pd::Patch patch = lpd.openPatch("control/_main-control.pd", "./pd");
     std::cout << patch << std::endl;
   } else {
     pd::Patch patch = lpd.openPatch("audio/_main-audio.pd", "./pd");
     std::cout << patch << std::endl;
   }
...
// JD: this is the main part of the program. The first argument sets the audio device number. If no argument given, then this sets to default.
